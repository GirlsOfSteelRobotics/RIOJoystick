...
/**
 * Handle input from standard Joysticks connected directly to the RoboRIO. Based
 * on the WPI Libary {@link edu.wpi.first.wpilibj.Joystick Joystick} class.
 *
 * <p>
 * Provides access to a Joystick plugged into a USB port on the RoboRIO.
 */
public class RIOJoystick extends RIOGenericHID {
    ...
    /**
     * Construct an instance of a joystick. The joystick index is the USB device
     * number on the RoboRIO, starting from zero and incrementing for each device
     * attached.
     *
     * @param deviceNum The device number for the joystick on the RoboRIO
     */
    public RIOJoystick(final int deviceNum) {
        super(deviceNum);
